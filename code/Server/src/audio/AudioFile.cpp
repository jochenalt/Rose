...
bool AudioFile<T>::setAudioBuffer (AudioBuffer& newBuffer)
{
    ...
    {
        assert (newBuffer[k].size() == (unsigned)numSamples);
...
bool AudioFile<T>::decodeAiffFile (std::vector<uint8_t>& fileData)
{
    ...
    if ((soundDataChunkSize - 8) != totalNumAudioSampleBytes || (unsigned)totalNumAudioSampleBytes > (fileData.size() - (unsigned)samplesStartIndex))
    {
...
bool AudioFile<T>::saveToWaveFile (std::string filePath)
{
    ...
    if ((unsigned)fileSizeInBytes != (fileData.size() - 8) || dataChunkSize != (getNumSamplesPerChannel() * getNumChannels() * (bitDepth / 8)))
    {
...
bool AudioFile<T>::saveToAiffFile (std::string filePath)
{
    ...
    // check that the various sizes we put in the metadata are correct
    if ((unsigned)fileSizeInBytes != (fileData.size() - 8) || soundDataChunkSize != getNumSamplesPerChannel() *  numBytesPerFrame + 8)
    {
        std::cout << "ERROR: couldn't save file to " << filePath << std::endl;
        return false;
    }
...
int AudioFile<T>::getIndexOfString (std::vector<uint8_t>& source, std::string stringToSearchFor)
{
    ...
    for (unsigned i = 0; i < source.size() - stringLength;i++)
    {
