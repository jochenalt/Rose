...
// the following c-code is from https://gist.github.com/andrewrk
...
void SoundCardUtils::setup() {
    ...
    if (pa_get_devicelist(pa_input_devicelist, pa_output_devicelist) < 0) {
		cerr << "ERR: failed to get device list" << endl;
		exit(1);
    }
    ...
    for (ctr = 0; ctr< 16; ctr++) {
		...
		SoundCard device = { .name =  pa_output_devicelist[ctr].name,
    ...
    for (ctr = 0; ctr < 16; ctr++) {
		...
		SoundCard device = { .name =  pa_input_devicelist[ctr].name,
...
void SoundCardUtils::printSoundCards() {
	...
	cout << "audio input devices:" << endl;
	for (unsigned i = 0;i<inputDevices.size();i++) {
		SoundCard device = inputDevices[i];
		cout << "   " << device.index  << ":" << device.name << "," << device.descr << endl;
	}

}

SoundCard SoundCardUtils::getInputDevice(int idx) {
	for (unsigned i = 0;i<outputDevices.size();i++) {
		SoundCard device = outputDevices[i];
		if (device.index == idx)
			return device;
...
SoundCard SoundCardUtils::getOutputDevice(int idx) {
	for (unsigned i = 0;i<outputDevices.size();i++) {
		SoundCard device = outputDevices[i];
		if (device.index == idx)
			return device;
